...
window.onload = async function init(){
    ...
    Projects.setProjects(loaded == null ? [] : loaded as Projects.Project[])
...
function inputs(){
    let searchInput = document.querySelector<HTMLInputElement>(".search-box")
    searchInput.addEventListener("input" , function(event){
        searchInput = event.target as HTMLInputElement
...
function overlay(panelActive : string){
    ...
    allPanel.forEach(panel => {
        let npanel : any = document.querySelector("." + panel)
    ...
    let overlayPanel : any = document.querySelector(".modal-overlay")
...
function buttons(){
    ...
    document.querySelector("#locate-btn").addEventListener("click" , function(){
        fetch("/locate").then(resp => resp.json()).then(data => {
            if (data.success){ 
                let pathInput = document.querySelector("#project-add-path") as HTMLInputElement
    ...
    document.querySelector("#locate-btn-project-folder").addEventListener("click" , function(){
        fetch("/locate").then(resp => resp.json()).then(data => {
            if (data.success){ 
                let pathInput = document.querySelector("#projects-folder") as HTMLInputElement
                pathInput.value = data.content
            }
        })
    })
    ...
    document.querySelectorAll(".filter-btn").forEach((element : HTMLButtonElement)  => {
        element.addEventListener("click" , function(){
            // Alle normal
            document.querySelectorAll(".filter-btn").forEach((el : HTMLButtonElement) => {
            ...
            let category : number = parseInt(element.dataset.value)


            if (category == -1){
                filterIde = null 
            }else{
                filterIde = category as Projects.ide
            }
            projectUi()
...
function editProject(project : Projects.Project){
    document.querySelector<HTMLElement>(".project-edit-modal .close-btn").onclick = async function(){
        overlay("project-view-modal")
        await projectView(project)
    }

    document.querySelector<HTMLInputElement>("#project-edit-name").value = project.name

    document.querySelectorAll<HTMLInputElement>(".ide-option.edit").forEach(element => {
    ...
    document.querySelector<HTMLInputElement>("#project-edit-icon").value = project.icon

    // Confirm project
    document.querySelector<HTMLElement>(".project-edit-modal .create-project-confirm").onclick = function(){
        saveProject(project)
    }
    
}

function saveProject(project : Projects.Project){
    let projectName = document.querySelector<HTMLInputElement>("#project-edit-name").value
    let projectIde : Projects.ide = parseInt(document.querySelector<HTMLElement>(".ide-option.edit.selected").dataset.value) as Projects.ide
    let projectIcon = document.querySelector<HTMLInputElement>("#project-edit-icon").value
...
function addTemplate(){
    let templateName = document.querySelector<HTMLInputElement>("#template-add-name").value
    let templateDescription = document.querySelector<HTMLInputElement>("#template-add-des").value
    let templateCmd = document.querySelector<HTMLInputElement>("#template-add-cmd").value
    let templateIcon = document.querySelector<HTMLInputElement>("#template-icon").value
...
async function createProject(){
    let projectInput = document.querySelector<HTMLInputElement>("#project-name");
    let projectName: string | undefined = projectInput?.value; 
    ...
    if (projectName != undefined){
        ...
        let ideSelect : any =  document.querySelector(".ide-option.selected")
        let ideNum : number = parseInt(ideSelect.dataset.value) + 1
       
        let ide : Projects.ide = ideNum as Projects.ide;
...
async function openProject(project : Projects.Project){
...
function deleteProject(project : Projects.Project ){
...
function settingsView(){
    let panel = document.querySelector<HTMLElement>(".settings-modal")
    
    panel.querySelector<HTMLInputElement>("#projects-folder").value = settings.get("projectsFolder")
    panel.querySelector<HTMLInputElement>("#use-categories").checked = settings.get("categories")
}

function saveSettings(){
    let panel = document.querySelector<HTMLElement>(".settings-modal")

    let projectsFolder = panel.querySelector<HTMLInputElement>("#projects-folder").value
    settings.set("projectsFolder" , projectsFolder)
    

        
    let useCategories = panel.querySelector<HTMLInputElement>("#use-categories").checked
...
async function addProject(){
    ...
    let ide : Projects.ide = Projects.ide.VSCODE
...
async function templateSelect(){
    ...
    let templateSelect = document.querySelector<HTMLElement>(".project-template")
    let prefab = templateSelect.querySelector<HTMLElement>(".option")
    templates.forEach(element => {
        let clone = prefab.cloneNode(true) as HTMLElement
    ...
    templateSelect.addEventListener("click" , function(event){
        ...
        if (target.classList.contains("active-option")){
            templateSelect.querySelector<HTMLElement>(".dropdown").style.display = "flex"
            templateSelect.style.userSelect = "none"
        }
    })
    templateSelect.querySelectorAll(".option").forEach(option => {
        let text = option.querySelector<HTMLElement>("h4").textContent
        option.addEventListener("click" , function(){
            
            templateSelected = text
            templateSelect.querySelector<HTMLElement>(".dropdown").style.display = "none"

            templateSelect.style.userSelect = ""
            templateSelect.querySelector<HTMLElement>(".active-option h4").textContent = templateSelected
            templateSelect.querySelector<HTMLImageElement>(".active-option img").src =  option.querySelector<HTMLImageElement>("img").src
        })
    });
}
...
async function sortSelect(){
    ...
    let sortSelect = document.querySelector<HTMLElement>(".sort-after")


    sortSelect.addEventListener("click" , function(event){
        let target : HTMLElement = event.target as HTMLElement
        if (target.classList.contains("active-option")){
            sortSelect.querySelector<HTMLElement>(".dropdown").style.display = "flex"
            sortSelect.style.userSelect = "none"
        }
    })

    sortSelect.querySelector<HTMLElement>(".active-option h4").textContent ="Sortieren nach: " + sortAfter
    ...
    sortSelect.querySelectorAll(".option").forEach(option => {
        ...
        option.addEventListener("click" , function(){
            ...
            sortAfter = text
            sortSelect.querySelector<HTMLElement>(".dropdown").style.display = "none"
...
async function projectView(project : Projects.Project){
    ...
    document.querySelector<HTMLElement>(".view-project-remove").onclick = function(){
    ...
    document.querySelector<HTMLElement>(".view-project-delete").onclick = function()
    {
    ...
    panel.querySelector<HTMLElement>(".path-card").onclick = function(){
    ...
    panel.querySelector<HTMLImageElement>(".current-icon img").src = iconUrl
    ...
    // Edit project
    document.querySelector<HTMLButtonElement>(".btn-project-edit").onclick = function(){
    ...
    info.languages.forEach(language => {
        ...
        let progressFill = clone.querySelector(".progress-fill") as HTMLElement;
    ...
    // Open Project
    let openBtn = document.querySelector<HTMLButtonElement>(".view-project-open");
    if (openBtn ) {
        openBtn.onclick = () => {
            openProject(project);
        };
...
function projectUi(){
    ...
    if (filterIde != null)
    {
        filtered = Projects.filterProjects({"ide" : filterIde as Projects.ide})
    }
    ...
    filtered.forEach(project => {
        ...
        if (clone){
            ...
            clone.querySelector<HTMLImageElement>(".project-icon").src = iconUrl
            let iconElement = clone.querySelector<HTMLImageElement>(".project-icon img")
